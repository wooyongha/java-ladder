...
public class Ladder {
    ...
    public LadderResult playLadderGame(final GameAttendees gameAttendees, final LadderCompensation ladderCompensation) {
        ...
        for (int startPos = 0; startPos < gameAttendees.getNumberOfGameAttendees(); startPos++) {
            final String attendees = gameAttendees.getGameAttendees(startPos);
            final String attendeesFinalCompensation = ladderCompensation.getCompensation(getAttendeesFinalPosition(startPos));
            gameResult.put(attendees, attendeesFinalCompensation);
        }
